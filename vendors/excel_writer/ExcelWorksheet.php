...
class ExcelWorksheet extends BIFFwriter
{
    /**
    ...
    * @access private
    */
    function ExcelWorksheet($BIFF_version, $name,
                                                $index, &$activesheet,
    ...
    */
    function setZoom($scale = 100)
    {
        ...
        if ($scale < 10 || $scale > 400) {
            trigger_error("Zoom factor $scale outside range: 10 <= zoom <= 400", E_USER_ERROR);
            $scale = 100;
        }
    ...
    */
    function setPrintScale($scale = 100)
    {
        // Confine the scale to Excel's range
        if ($scale < 10 || $scale > 400) {
            trigger_error("Print scale $scale outside range: 10 <= zoom <= 400", E_USER_ERROR);
            $scale = 100;
        }
    ...
    */
    function write($row, $col, $token, $format = null)
    {
        ...
        if (is_array($token)) {
            // Match number
            return $this->writeRow($row, $col, $token, $format);
        } 
    ...
    */
    function setInputEncoding($encoding)
    {
         if ($encoding != 'UTF-16LE' && !function_exists('iconv')) {
             //trigger_error("Using an input encoding other than UTF-16LE requires PHP support for iconv", E_USER_ERROR);
         }
    ...
    */
    function _processBitmap($bitmap)
    {
        ...
        if (!$bmp_fd) {
            trigger_error("Couldn't import $bitmap", E_USER_ERROR);
        }
        ...
        if ($identity['ident'] != "BM") {
            trigger_error("$bitmap doesn't appear to be a valid bitmap image.\n", E_USER_ERROR);
        }
        ...
        $data   = substr($data, 8);
        if ($width > 0xFFFF) {
            trigger_error("$bitmap: largest image width supported is 65k.\n", E_USER_ERROR);
        }
        if ($height > 0xFFFF) {
            trigger_error("$bitmap: largest image height supported is 65k.\n", E_USER_ERROR);
        }
