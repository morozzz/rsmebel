...
require_once 'ExcelFormat.php';
require_once 'BIFFwriter.php';
require_once 'ExcelWorksheet.php';
...
class ExcelWorkbook extends BIFFwriter
{
    /**
    ...
    * @access public
    */
    function ExcelWorkbook($filename)
    {
        // It needs to call its parent's constructor explicitly
        $this->BIFFwriter();

        $this->_filename         = $filename;
        $this->_parser           =& new ExcelParser($this->_byte_order, $this->_BIFF_version);
        $this->_1904             = 0;
        $this->_activesheet      = 0;
        $this->_firstsheet       = 0;
        $this->_selected         = 0;
        ...
        $this->_tmp_format       =& new ExcelFormat($this->_BIFF_version);
    ...
    */
    function &addWorksheet($name = '')
    {
        ...
        $worksheet = new ExcelWorksheet($this->_BIFF_version,
                                   $name, $index,
    ...
    * @return &ExcelFormat reference to an Excel Format
    */
    function &addFormat($properties = array())
    {
        $format = new ExcelFormat($this->_BIFF_version, $this->_xf_index, $properties);
        $this->_xf_index += 1;
    ...
    {
        include_once 'ExcelValidator.php';
        /* FIXME: check for successful inclusion*/
        $valid = new ExcelValidator($this->_parser);
        return $valid;
    }
    ...
    */
    function setCustomColor($index, $red, $green, $blue)
    {
        ...
        if ($index < 8 or $index > 64) {
            ...
            trigger_error("Color index $index outside range: 8 <= index <= 64", E_USER_ERROR);
        }
        ...
        if (($red   < 0 or $red   > 255) ||
            ...
            ($blue  < 0 or $blue  > 255))
        {
            trigger_error("Color component outside range: 0 <= color <= 255", E_USER_ERROR);
        }
    ...
    */
    ...
    {
        $OLE = new OleFile(Ole::Asc2Ucs('Book'));
        if ($this->_tmp_dir != '') {
        ...
        $root = new OleRoot(time(), time(), array($OLE));
        if ($this->_tmp_dir != '') {
